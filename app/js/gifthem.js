    ...
    if (window.File && window.FileReader && window.FileList && window.Blob) {
        ...
        document.querySelector('#filesToUpload').onchange = function(){
            ...
            document.querySelector('#gifTheme').onclick = function() {
                var imgTags = Array.apply(null, document.querySelectorAll('#filesInfo .crop-image img'));
                var imgList = [];
    ...
    function resizeAndCrop(file, MAX_WIDTH, MAX_HEIGHT) {
        ...
        reader.onloadend = function() {
            ...
            tempImg.onload = function() {
                ...
                div.id = id;
                div.innerHTML = '' +
                    '<a class="crop-image" href="javascript:void(0);" onclick="cropImage(\'' + popup + '\', \'' + img + '\', \'' + preview + '\');" rel="nofollow">' +
                        '<img id="' + mainImg + '" src="' + dataURL + '" alt="" />' +
                    '</a>' +
                    '<span class="controls">' +
